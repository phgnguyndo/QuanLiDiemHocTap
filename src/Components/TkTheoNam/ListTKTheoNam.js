import { TableContainer, useDisclosure, Select } from "@chakra-ui/react";
  ...
  useEffect(() => {
    const tinhDiemTrungBinhNamHoc = (dsHV, dsDTB) => {
      ...
      for (let i = 0; i < dsHV.length; i++) {
        ...
        temp =
          dsDTB[j].dtb * dsDTB[j].tongTC +
          dsDTB[j + 1].dtb * dsDTB[j + 1].tongTC;
        tcNam = dsDTB[j].tongTC + dsDTB[j + 1].tongTC;
        hv_diem[i][1] = tcNam > 0 ? temp / tcNam : 0;
        temp =
          dsDTB[j + 2].dtb * dsDTB[j + 2].tongTC +
          dsDTB[j + 3].dtb * dsDTB[j + 3].tongTC;
        tcNam = dsDTB[j + 2].tongTC + dsDTB[j + 3].tongTC;
        hv_diem[i][2] = tcNam > 0 ? temp / tcNam : 0;

        temp =
          dsDTB[j + 4].dtb * dsDTB[j + 4].tongTC +
          dsDTB[j + 5].dtb * dsDTB[j + 5].tongTC;
        tcNam = dsDTB[j + 4].tongTC + dsDTB[j + 5].tongTC;
        hv_diem[i][3] = tcNam > 0 ? temp / tcNam : 0;

        temp =
          dsDTB[j + 6].dtb * dsDTB[j + 6].tongTC +
          dsDTB[j + 7].dtb * dsDTB[j + 7].tongTC;
        tcNam = dsDTB[j + 6].tongTC + dsDTB[j + 7].tongTC;
        hv_diem[i][4] = tcNam > 0 ? temp / tcNam : 0;

        temp =
          dsDTB[j + 8].dtb * dsDTB[j + 8].tongTC +
          dsDTB[j + 9].dtb * dsDTB[j + 9].tongTC;
